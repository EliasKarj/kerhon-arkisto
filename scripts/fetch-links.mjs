// Build-time-skripti: hakee kullekin sarjalle suoratoistolinkit (AniListin
// externalLinks, type STREAMING) ja AniList-sivun osoitteen. Kirjoittaa
// data/links.json:iin muodossa { seriesId: { anilist, streaming: [{site,url,color}] } }.
//
// Aja projektin juuresta:  npm run fetch:links
...
async function main() {
  ...
  const outPath = join(DATA_DIR, "links.json");
  writeFileSync(outPath, JSON.stringify(links, null, 2) + "\n", "utf8");
  console.log(`\nKirjoitettu ${Object.keys(links).length} sarjan linkit -> ${outPath}`);
}
